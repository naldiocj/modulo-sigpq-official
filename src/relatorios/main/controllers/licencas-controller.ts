...
import { Options } from '../../../../@core/domain/entity';
...
export default class Controller {
  ...
  tiposCabecalho() {
    return [
      ...
      {
        id: 'sigla', text: 'Orgão'.toUpperCase(), largura: 10
      }/* ,
      {
  ...
  public async listarTodos({ auth, request, response }: HttpContextContract): Promise<any> {
    ...
    try {
      ...
      const listaFuncionarios = await this.#licenca.listarTodosComPessoas(options)
     ...
     //console.log("Dados do Comandante do orgão:",comandante_do_orgao)
      ...
      doc.text("LICENÇA "+input.licenca_name.toUpperCase(), pageWidth / 2, 80, { align: "center" });
      ...
      function getMonthName(monthNumber: number): string {
        ...
        // Retorna o nome do mês, ajustando para o índice do array (0-based)
        return months[monthNumber - 1];
      }

      doc.text(`RELAÇÃO NOMINAL DOS EFECTIVOS EM LICENÇA ${input.licenca_name.toUpperCase()} DO ANO DE ${input.ano} DE ${getMonthName(input.mes).toUpperCase()}.`, 10, 90);
