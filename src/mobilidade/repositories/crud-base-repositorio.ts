...
export default class CrudBaseRepository extends BaseModuloRepository {
  ...
  public async registar(input: any, file: any, trxParam = null): Promise<any> {
    ...
    try {
      ...
      fileGuiaPath = fileGuia ? await uploadFile(pessoa_id + '/individual', fileGuia) : null;
      ...
      if (input.situacao == 'actual' || input.situacao == 'anterior') {
        ...
        for (const agente of agentes) {
          ...
          const pjd_id = input?.pessoajuridicaPassadoId ? input?.pessoajuridicaPassadoId : pjp?.pessoajudidica_id_passado

          const dp: any = await this.buscarOrgaoAnterior(agente, 'alto')
          const dpp_id = input?.departamentoPassadoId ? input?.departamentoPassadoId : dp?.pessoajudidica_id_passado
          ...
          const sc_id = input?.seccaoPassadoId ? input?.seccaoPassadoId : sc?.pessoajudidica_id_passado

          const posto: any = await this.buscarOrgaoAnterior(agente, 'baixo')
          const posto_id = input?.postoPassadoId ? input?.postoPassadoId : posto?.pessoajudidica_id_passado


          const guia = await this.getNumeroGuia(input.pessoajuridica_id, pjd_id)

          if (guia instanceof Error)
            return Error(guia.message)
          ...
          const mobilidade = {
            ...
            despacho_descricao: input?.despacho ? extensao_despacho(input?.data_despacho, input?.despacho) : null,
            despacho_data: input?.data_despacho,
          ...
          const mobilidadeId = await Database
          ...
          await Database.insertQuery().useTransaction(trx).table('sigpq_mais_mobilidades').insert(maisMobilidade)
          guiaAntiga = guia
  ...
  public async editarData(input: any, id: number, trx: any = null): Promise<any> {
    ...
    const numero_guia=`${numero_guia_aux[0]}/${numero_guia_aux[1]}/${numero_guia_aux[2]}/${data.getFullYear()}`
    const despacho_aux=input.despacho.split('/') || []
    const despacho= despacho_aux.length>1?`${input.numero_despacho}/${despacho_aux[1]}`:input.numero_despacho

    const despacho_descricao=`${extensao_despacho(input?.despacho_data,despacho)}`
    try {
  ...
  public async listarTodos(options: any): Promise<any> {
    try {
      ...
      let despachos = Database.from({ fo: 'sigpq_funcionario_orgaos' })
        ...
        .innerJoin('pessoajuridicas', 'pessoajuridicas.id', 'fo.pessoajuridica_id')
        .innerJoin('pessoajuridicas as passado', 'passado.id', 'fo.pessoajuridica_passado_id')
        ...
        if (options?.user?.aceder_todos_agentes) {
          ...
          if (options.tipoOrgaoId) {
            ...
            query.orWhere('passado.tipo_estrutura_organica_sigla', options.orgaoId)
          }
      ...
      if (options.page) {
        ...
        pagination['rows'] = await Promise.all(pagination['rows'].map(async (item: any) => {
          ...
          const colocacao = await this.buscarColocacao(item.pessoajuridica_id)
          const colocacao_passado = await this.buscarColocacao(item.pessoajuridica_passado_id)
          const user = await this.buscarUtilizador(item?.user_id)
          const responsavel = await this.mobilidadeResponsavel(item?.user_id)
          const tratamento: any = await Database.from({ t: 'sigpq_tratamento_mobilidades' })
            .select('t.*')
            .where('t.numero_guia', item?.numero_guia)
            .where('t.activo', true)
            .first()

          const agentes: any = await this.agentePorGuia({ numero_guia: item?.numero_guia })
          delete item.user_id
      ...
      let query: DatabaseQueryBuilderContract = Database.from({ fo: 'sigpq_funcionario_orgaos' })
        .select(
          "fo.*",
          // "fo.id",
          // "fo.pessoajuridica_id",
  ...
  public async listar(options: any): Promise<any> {
    try {
      let despachos = Database.from({ f: 'sigpq_funcionarios' })
        ...
        .innerJoin('sigpq_funcionario_orgaos as fo', 'fo.pessoafisica_id', 'pf.id')
      ...
      if (options.page) {
        ...
        pagination['rows'] = await Promise.all(pagination['rows'].map(async (item: any) => {
          ...
          const colocacao_passado = await this.buscarColocacao(item?.pessoajuridica_passado_id)
          const user = await this.buscarUtilizador(item.user_id)
          const responsavel = await this.mobilidadeResponsavel(item?.sigpq_funcionario_orgao_id)
  ...
  public async agentePorGuia(options: any): Promise<any> {
    try {
      let query = Database.from({ f: 'sigpq_funcionarios' })
        ...
        .innerJoin('sigpq_funcionario_orgaos as fo', 'fo.pessoafisica_id', 'f.id')
  ...
  public async listarTodosPorPessoa(options: any): Promise<any> {
    try {
      ...
      let despachos: DatabaseQueryBuilderContract = Database.from({ fo: 'sigpq_funcionario_orgaos' })
        .select(
          "fo.id",
          "fo.pessoajuridica_id",
          "fo.user_id",
        ...
        .leftJoin('pessoajuridicas as pjd', 'pjd.id', 'fo.pessoajuridica_passado_id')
      ...
      return options.page ? await despachos.paginate(options.page, options.perPage || 10) : await despachos
  ...
  public async buscarOrgaoAnterior(pessoafisica_id: any, nivel: string) {
    return pessoafisica_id ? await Database.from({ o: 'sigpq_funcionario_orgaos' })
  ...
  public async getNumeroGuia(pessoajuridicaId: any, pessoajuridicaPassadoId: any = null) {

    const anoGuia = this.dateTime.getFullYear()
    const orgao = await this.buscarColocacao(pessoajuridicaId)
    const orgaoPassado = pessoajuridicaPassadoId ? await this.buscarColocacao(pessoajuridicaPassadoId) : null
    if (!orgao)
      return Error('Não foi possível gerar guia')

    const siglaOgao = orgaoPassado ? this.getSiglaPNA(orgaoPassado?.sigla) + '/' + this.getSiglaPNA(orgao?.sigla) : this.getSiglaPNA(orgao?.sigla)
    const guia = `${this.numeroAutomatico + 1}/${siglaOgao}/${anoGuia}`;

    if (await this.buscarNumeroGuia(guia, pessoajuridicaId)) {
      this.getNumeroGuia(pessoajuridicaId, pessoajuridicaPassadoId)
      this.gerarNumeroOutro()
    }
    return guia;
  }

  public async buscarNumeroGuia(guia: any, pessoajuridicaId: any): Promise<any> {
    return guia ? await Database.from({ f: 'sigpq_funcionario_orgaos' }).select('*').where('numero_guia', guia).where('pessoajuridica_id', '<>', pessoajuridicaId).first() : null
  }
