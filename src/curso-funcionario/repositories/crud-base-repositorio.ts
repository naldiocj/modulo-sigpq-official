...
export default class CrudBaseRepository extends BaseModuloRepository {
  ...
  public async registar(input: any, file: any, trxParam = null): Promise<any> {
    let validate = []
    try {
      const camposDesejados: (keyof any)[] = ['pessoafisica_id', 'user_id', 'sigpq_tipo_curso_id','instituicao_de_ensino_id',
        'ano_de_conclusao_do_curso','activo'];
  ...
  public async listarUm(id: any): Promise<any> {
    ...
    try {
      ...
      .leftJoin('sigpq_tipo_formacao', 'sigpq_tipo_formacao.id', 'tc.tipo_formacao_id')
      .leftJoin('sigpq_instituicao_de_ensino', 'sigpq_instituicao_de_ensino.id', 'c.instituicao_de_ensino_id')
      .where('c.id', id)
  ...
  public async listarTodos(options: any): Promise<any> {
    try {
      ...
      const query: DatabaseQueryBuilderContract = Database.from({ c: 'sigpq_cursos' })
      .select(
