...
const tipoFuncaoRepository = new BaseRepository<IInput>('sigpq_tipo_carreiras');
...
export default class CrudRepository {
  ...
  public async registar(input: IInput, trx = null): Promise<any> {
    ...
    try {
      ...
      console.log(e);
      return Error('Não foi possível criar um novo tipo de carreira.');
    }
  }
  ...
  public async listarTodos(options: any): Promise<any> {
    ...
    try {
      ...
      let query: DatabaseQueryBuilderContract = Database.from({ s: 'sigpq_tipo_carreiras' })
        .select(
          's.id',
          's.nome',
          's.sigla',
        ...
        .where(query => {
          if (options.regime_id) {
            query.where('s.regime_id', options.regime_id)
          }
        })
        .orderBy('s.ordem')
