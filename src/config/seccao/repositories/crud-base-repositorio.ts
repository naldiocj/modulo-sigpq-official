    ...
    try {
      ...
      let query: DatabaseQueryBuilderContract = Database.from({ pj: 'pessoajuridicas' })
        .select(
          ...
          Database.raw('upper(pjo.sigla) as departamento_sigla'),
          Database.raw('upper(po.nome_completo) as departamento'),
          'po.id as departamento_id',

          Database.raw('upper(pjoo.sigla) as orgao_sigla'),
          Database.raw('upper(poo.nome_completo) as orgao'),
          'pjoo.id as orgao_id',
          'pjoo.tipo_estrutura_organica_sigla',
        ...
        //.whereIn('tpj.nome', ['Secção', 'Esquadra', 'Subunidade'])
        .where(query => {
          if (options.departamentoId) {

            query.where('pj.pessoajuridica_id', options.departamentoId)
          }
        })
        .where(query => {
          if (options.orgaoId) {
            query.where('pjo.pessoajuridica_id', options.orgaoId)

          }
        })
        .where(query => {
          if (options.tipoOrgao) {
            query.where('pjoo.tipo_estrutura_organica_sigla', options.tipoOrgao)
