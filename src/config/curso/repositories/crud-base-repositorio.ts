...
const tipoCursoRepository = new BaseRepository<Input>("sigpq_tipo_cursos");
...
export default class CrudBaseRepository extends BaseModuloRepository {
  ...
  public async registar(input: Input, trx: any = null): Promise<any> {
    ...
    try {
      ...
      if (cursoPorNome) return Error("Curso com este nome já existe");

      if (cursoPorSigla) return Error("Curso com esta sigla já existe");
  ...
  public async editar(input: Input, id: number, trx: any = null): Promise<any> {
    ...
    try {
      ...
      await Database.from("sigpq_tipo_cursos")
        .where("id", id)
        .update(valido)
        .useTransaction(trx);
  ...
  public async listarTodos(options: any): Promise<any> {
    try {
      ...
      //     query.where("s.tipo_formacao_id", options.tipo_formacao_id);
      //   }
      // });
