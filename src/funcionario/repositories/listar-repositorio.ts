...
export default class CrudBaseRepository {
  ...
  public async _listarTodos(options: any): Promise<any> {
    ...
    try {
      ...
      const query = mysql2.from('sigpq_funcionarios as f').select(
        ...
        mysql2.raw("DATE_FORMAT(f.created_at, '%d/%m/%Y %H:%i:%s') as created_at"),
        mysql2.raw("DATE_FORMAT(f.updated_at, '%d/%m/%Y %H:%i:%s') as updated_at"),
        mysql2.raw("DATE_FORMAT(fe.data_inicio_inatividade, '%d/%m/%Y %H:%i:%s') as data_inicio_inatividade"),
  ...
  private construirQueryBase(trx: any) {
    return Database.from({ f: 'sigpq_funcionarios' })
      .useTransaction(trx)
      ///.distinct('f.id')
      .select(
  ...
  private aplicarFiltros(query: any, options: any) {
    ...
    if (options.sigpq_estado_reforma_id) {
      query.where('fe.sigpq_estado_reforma_id', Number(options.sigpq_estado_reforma_id));
    }
    ...
    if (options.aceder_todos_agentes) {
      ...
      if (options.aceder_departamento) {
        query.innerJoin('sigpq_funcionario_orgaos as orgao', 'orgao.pessoafisica_id', 'p.id')
    ...
    if (options.situacaoId == 5 && options.sigpq_excluir_estado_reforma_id && !options.estadoId) {
      query.whereNotIn('fe.sigpq_estado_reforma_id', options.sigpq_excluir_estado_reforma_id);
    }

    if (options.excluir_efectividade_e_inactividade) {
      query.whereNotIn('fe.sigpq_situacao_id', options.excluir_efectividade_e_inactividade);
    }
  ...
  private filtrarPorSearch(query: any, search: string) {
    query.where((item: any) => {
      ...
      item.orWhereRaw("concat(p.nome_completo, ' ', pf.apelido) like ?", `%${search}%`);
  ...
  public async buscarNip(n: any, pessoaId: any = null) {
    return n ? await Database.from({ f: this.#table })
