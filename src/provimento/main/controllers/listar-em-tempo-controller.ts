...
import ListarEmTempoProvimentoRepository from "../../repositories/listar-em-tempo-repositorio";
import { useLogger } from "App/Helper/logger";

const { ok } = require("App/Helper/Http-helper");

export default class ListarEmTempoController {
  #repo: ListarEmTempoProvimentoRepository;

  redis = new RedisService();
  constructor() {
    this.#repo = new ListarEmTempoProvimentoRepository();
  }
  ...
  public async execute({
    ...
    const options = {
      ...
      patenteClasse: await this.validarNullOuUndefined(
        request,
        "patenteClasse"
      ),
      sigpq_acto_progressao_id: await this.validarNullOuUndefined(
        request,
        "sigpq_acto_progressao_id"
      ),
      tipoVinculoId: await this.validarNullOuUndefined(
        request,
        "tipoVinculoId"
      ),
    ...
    useLogger(request, auth, "actualizar em tempo", "provimentos");

    return ok(result, null);
  }

  async validarNullOuUndefined(request: any, field: any) {
    return ["null", "0", undefined].includes(request.input(field))
      ? null
      : request.input(field);
  }
}
